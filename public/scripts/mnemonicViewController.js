configApp.controller('mnemonicViewController', function($scope, $location, dataTransfer) {

    $(window).on("unload", function(e) {
        console.log("store local lists into storage");
        sessionStorage.setItem('qa', JSON.stringify(dataTransfer.getQA()));
        sessionStorage.setItem('mnemonics', JSON.stringify($scope.mnemonics));
    });
        ...
        $.ajax({
           ...
           success: function (data) {
               $scope.$apply(function(){
                ...
                var table = $('#mnemonicList').DataTable({
                    lengthChange: false,
                    pageLength: 10
                }); 
                $('#loading').hide();
           ...
           // var local = sessionStorage.getItem('mnemonics');
    ...
    // function that merges two lists based on duplicate properties
    ...
    // function that simply prints the log data for the delete button
    $scope.logLength = function() {
        console.log("load delete data");
    };
    ...
    $scope.loadModifyData = function() {
        ...
        if(document.getElementById('mnemonicNameField').placeholder === "mixed") { // each mnemonic has a unique name
            document.getElementById('modifyMnemonicNameCheck').disabled = true; // disable any attempt to make a change
    ...
    $scope.modifyModal = function() {
        ...
        if(document.getElementById('modifyMnemonicNameCheck').checked === true) {
            ...
            for(var i = 0; i < $scope.mnemonicEditList.length; i++) { // for each mnemonic we are modifying
            ...
            if($scope.mnemonicEditList.length < 2) {
                if(document.getElementById("mnemonicNameField").value === "") {
                    alert("Mnemonic Name Cannot Be Empty!"); // every mnemonic name needs to be unique and non-empty
                    return;
                }
            }
        }
    ...
    $scope.createModal = function() {
        if(document.getElementById("mnemonicName").value === "") {
            alert("Mnemonic Name Cannot Be Empty!");
        } else if(document.getElementById("mnemonicDisplay").value === "") {
            document.getElementById("mnemonicDisplay").style.borderColor = "red";
        } else {
            var mnemonic = {
